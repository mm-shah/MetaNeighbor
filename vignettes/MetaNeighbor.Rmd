...
, MetaNeighbor takes four inputs: a gene-by-sample expression matrix (âdataâ), a
set of labels indicating each sampleâs dataset of origin (âexperiment labelsâ),
a set of labels indicating each sampleâs cell type (âcell type labelsâ) and a
set of genes (âgene setsâ). The output is a performance vector (âAUROC scoresâ),
...
1.  A gene-by-sample expression matrix (âdataâ)
2.  A numerical vector that indicates the source of each sample (âexperiment
labelsâ)
3.  A matrix that indicates the cell type of each sample (âcell type labelsâ)
4.  Gene sets of interest (âgene setsâ) provided as a list of vectors
...
sample (âphenoâ)
3.  A vector that lists the names of all cell types (âcelltypesâ)
4.  A set of high variance genes (âvar.genesâ)
...
files. Specify the desired path and file name using the file\_ext variable (e.g. âfilenameâ)
...
AUROC score for each dataset, cell type and gene set can be found in the âfilename.IDscore.list.RDataâ file that will be saved, and matrix containing the
mean score for each cell type can be found in âfilename.IDscore.matrix.Rdataâ.
...
1.  A gene-by-sample expression matrix (âdataâ)
2.  A sample metadata table, that lists the dataset and cell type for each
sample with column names âStudy\_IDâ and âCelltypeâ (âphenoâ)
3.  A vector that lists the names of all cell types (âcelltypesâ)
4.  A set of high variance genes (âvar.genesâ)
...
the best hit for one another (âreciprocal top hitsâ), and cell types with scores
...
that are âpromiscuousâ (i.e., have broad similarity to many types) will tend to
